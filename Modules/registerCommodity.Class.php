...
class registerCommodity extends Settings{
    ...
    function confirm($user_id,$tax,$amount,$price,$total,$image_dir,$shoppingnum,$memo,$name){
        ...
        $insert_sql_2 = "INSERT INTO orders(commodity_ID,user_id,tax,amount,price,total,image,purchased,shopped,shoppingnum,memo,del_flg) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
        $stmt = $this->pdo->prepare($insert_sql_2);
        $result3 = $stmt->execute(array($result2,$user_id,$tax,$amount,$price,$total,$image_dir,0,0,$shoppingnum,$memo,0));
